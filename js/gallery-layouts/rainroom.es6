...
export class RainRoom extends GalleryLayout {
  constructor(options) {
    ...
    this.timeToReachMaxParticleTarget =
      options.timeToReachMaxParticleTarget || 600 * 1000; // 10 minutes
    this.rainParticleSize = options.rainParticleSize || 1;
    this.initialRainParticleSizeDelay =
      options.initialRainParticleSizeDelay || 45 * 1000;
    ...
    this.timeBetweenRaindropsDecrement =
      options.timeBetweenRaindropsDecrement || 30; // number of ms to deceremnt time between raindrops on every rain fall
    this.raindropSizeVariance = options.raindropSizeVariance || 1;
    this.raindropSizeVarianceGrowthRate =
      options.raindropSizeVarianceGrowthRate || 1.005;
    this.raindropSizeVarianceIncrement =
      options.raindropSizeVarianceIncrement || 0.035; // number of "space units" to increment raindrop size variance each time raindrop is createed
    this.raindropMaxRadius = options.raindropMaxRadius || 15;
    this.minimumTimeBetweenRaindrops =
      options.minimumTimeBetweenRaindrops || 200;
    ...
    this.timeToAddAlternativeMedia =
      options.timeToAddAlternativeMedia || 180 * 1000;
    this.ghostSizeVariance = options.ghostSizeVariance || 20;
    this.ghostSizeVarianceGrowthRate =
      options.ghostSizeVarianceGrowthRate || 1.004;
    this.ghostSizeVarianceIncrement =
      options.ghostSizeVarianceIncrement || 0.05; // number of "space units" to increment ghost size variance each time ghost is created
    this.maxGhostLength = options.maxGhostLength || 36;

    // constriction config
    this.groundBeginToRiseDelay =
      options.groundBeginToRiseDelay || this.removeRaindropTime + 60000; // 1 minute after drops are removed
    ...
    this.delayForRapidTimeBetweenWallUpdates =
      options.delayForRapidTimeBetweenWallUpdates ||
      this.groundBeginToRiseDelay + this.groundAscensionTime - 60000;
    this.minimumTimeBetweenWallUpdates =
      options.minimumTimeBetweenWallUpdates || 10;
    this.wallMediaIndex = 0;

    // particle texture config
    this.delayForImagesAsRainParticles =
      options.delayForImagesAsRainParticles || this.removeRaindropTime;
    this.timeBetweenRainParticleImages =
      options.timeBetweenRainParticleImages || 45000;
    this.minimumTimeBetweenRainParticleUpdates =
      options.minimumTimeBetweenRainParticleUpdates || 10000;

    // jump config
    this.jumpLevels = options.jumpLevels || [
      { delay: 300000, boost: 150 },
      { delay: 800000, boost: 200 },
      { delay: 1000000, boost: 250 }
    ];
    ...
    if (!this.domMode) {
      ...
      this.ground = createPlane(
        ...
        otherObject => {
          ...
          if (
            otherObject._media &&
            !this.rainCollisionSet[otherObject._media.id]
          ) {
      ...
      this.ceiling = createPlane(
        this.roomLength,
        this.yLevel + this.wallHeight,
        ceilingMaterial
      );
  ...
  start() {
    ...
    if (!this.domMode) {
        ...
        this.particleSizeInterval = setInterval(() => {
          if (this.rainParticleSize < this.maxParticleSize) {
            this.rainParticleSize =
              this.rainParticleSize + this.rainParticleSizeIncrement;
      ...
      this.particleGrowthInterval = setInterval(() => {
        ...
        var percentageOfMaxTimeElapsed =
          timeElapsed / this.timeToReachMaxParticleTarget;
        var totalParticlesToGrow =
          this.maxParticlesPerEmitter - this.initialParticlesPerEmitter;
        var currentNumberOfParticles =
          this.initialParticlesPerEmitter +
          Math.round(percentageOfMaxTimeElapsed * totalParticlesToGrow);
        console.log(
          "number of particles per emitter: " + currentNumberOfParticles
        );
        this.updateParticlesPerEmitter(
          Math.min(this.maxParticlesPerEmitter, currentNumberOfParticles)
        );
      ...
      setTimeout(() => {
        ...
        var tween = new TWEEN.Tween(groundPosition).to(
          ascensionTarget,
          this.groundAscensionTime
        );
      ...
      var updateWall = () => {
        ...
        if (this.flashingWallsRapidly) {
          this.timeBetweenWallUpdates = Math.max(
            this.minimumTimeBetweenWallUpdates,
            this.timeBetweenWallUpdates - 10
          );
          console.log(
            "time between wall updates: " + this.timeBetweenWallUpdates
          );
        }
      ...
      setTimeout(() => {
        var updateRainParticles = () => {
          ...
          this.rainParticleGroup.uniforms.texture.value = this.createTexture(
            media
          );

          this.timeBetweenRainParticleImages = Math.max(
            this.minimumTimeBetweenRainParticleUpdates,
            this.timeBetweenRainParticleImages - 2000
          );
          console.log(
            "time between rain particle updates: " +
              this.timeBetweenRainParticleImages
          );
    ...
    setTimeout(() => {
      this.particleSpreadInterval = setInterval(() => {
        if (this.particleSpread < this.maxParticleSpread) {
          this.particleSpread =
            this.particleSpread + this.rainParticleSpreadIncrement;
  ...
  update(dt) {
    ...
    var nearDistance =
      (this.spacePerEmitter / 1.5) * (this.spacePerEmitter / 1.5);

    for (var i = 0; i < this.emitters.length; i++) {
      var iEmitters = this.emitters[i];
      for (var j = 0; j < iEmitters.length; j++) {
        var emitter = iEmitters[j];
        var controlBelowThisEmitter =
          this.controlObject.position.distanceToSquared(emitter.__position) <
          nearDistance;
  ...
  layoutNextMedia() {
    ...
    if (this.useAcceleration) {
      this.timeBetweenRaindrops = Math.max(
        this.minimumTimeBetweenRaindrops,
        this.timeBetweenRaindrops * this.raindropTimeDecayRate
      );
    } else {
      this.timeBetweenRaindrops = Math.max(
        this.minimumTimeBetweenRaindrops,
        this.timeBetweenRaindrops - this.timeBetweenRaindropsDecrement
      );
    }
    setTimeout(() => {
      this.layoutNextMedia();
  ...
  layoutMedia(index, media) {
    ...
    if (this.domMode) {
      var $image = $(
        '<img style="display: block; position: absolute; background-color: white; z-index: 1000; box-shadow: 0 10px 20px rgba(0,0,0,0.19), 0 6px 6px rgba(0,0,0,0.23);"></img>'
      );
      var width =
        Math.random() * window.innerWidth * 0.4 + window.innerWidth * 0.08;
    ...
    if (!this.canAddAlternativeMedia || Math.random() < 0.67) {
      ...
      mesh.position.set(
        this.randomPointInRoom(),
        this.initialRaindropY,
        this.randomPointInRoom()
      );
    } else {
      var creators = [
  ...
  createRaindrop(media) {
    var radius = Math.min(
      this.raindropMaxRadius,
      Math.round(Math.random() * this.raindropSizeVariance + 2)
    );
  ...
  createGarbage(media) {
    var length = Math.min(
      this.maxGhostLength,
      Math.round(Math.random() * this.ghostSizeVariance + 2)
    );
    ...
    var geometry = parametricGeometries.createCrumpledGarbage({
      radius: length
    });
  ...
  setupRainParticleSystem() {
    this.rainParticleGroup = new SPE.Group({
      ...
      maxParticleCount:
        1 +
        this.maxParticlesPerEmitter *
          this.emittersPerWall *
          this.emittersPerWall,
      fog: true
    });
    ...
    for (var i = 0; i < this.emittersPerWall; i++) {
      ...
      for (var j = 0; j < this.emittersPerWall; j++) {
        ...
        var emitter = new SPE.Emitter({
          ...
          activeMultiplier:
            this.initialParticlesPerEmitter / this.maxParticlesPerEmitter // this is a value between 0 and 1 that sets how many particles actually used
        });
